...
export function ParameterCreatePeriods() {
	//
	...
	const currentPeriodIds = createParameterContext.data.form.values.periodIds || [];
	...
	const applyPreset = (key: null | PeriodPresetKey) => {
		if (!key) {
			...
			createParameterContext.data.form.setFieldValue('periodIds', undefined);
			return;
		}

		// select all
		createParameterContext.data.form.setFieldValue('periodIds', [...allPeriodIds]);
	};
		...
		<Section gap="md">
			...
			<SegmentedMultiSelect
				...
				onChange={(selectedPeriods) => {
					createParameterContext.data.form.setFieldValue('periodIds', selectedPeriods.length > 0 ? selectedPeriods : undefined);
				}}
