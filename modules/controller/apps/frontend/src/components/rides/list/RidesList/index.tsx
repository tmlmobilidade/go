	...
	const columns: DataTableColumn<RideNormalized>[] = [
		{
			...
			title: t('controller:rides.list.RidesListTable.columns.seen_last_at.label'),
			width: 24,
		},
		{
			accessor: 'operational_status',
			render: item => <OperationalStatusTag value={item.operational_status} />,
			title: t('controller:rides.list.RidesListTable.columns.operational_status.label'),
			width: 150,
		},
		{
			accessor: 'operational_date',
			render: item => <OperationalDateTag value={item.operational_date} />,
			title: t('controller:rides.list.RidesListTable.columns.operational_date.label'),
			width: 150,
		},
		{
			accessor: 'headsign',
			render: item => <RidesListCellHeadsign headsign={item.headsign} patternId={item.pattern_id} />,
			title: t('controller:rides.list.RidesListTable.columns.headsign.label'),
			width: 500,
		},
		{
			accessor: 'start_time_scheduled',
			render: item => <Tag label={formatTimestamp(item.start_time_scheduled)} variant="muted" />,
			title: t('controller:rides.list.RidesListTable.columns.start_time_scheduled.label'),
			width: 80,
		},
		{
			...
			title: t('controller:rides.list.RidesListTable.columns.start_time_observed.label'),
			width: 200,
		},
		{
			accessor: 'passengers_observed',
			render: item => <RidesListCellPassengers value={item.passengers_observed} />,
			title: t('controller:rides.list.RidesListTable.columns.passengers_observed.label'),
			width: 120,
		},
		{
