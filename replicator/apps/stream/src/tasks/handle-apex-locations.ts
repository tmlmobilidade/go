...
import { simplifyApexLocation } from '@tmlmobilidade/sae-replicator-pckg-parse';
...
const apexT3DbWritter = new MongoDbWriter<SimplifiedApexLocation>({
	batch_size: 250,
	batch_timeout: 10000,
...
export async function handleApexLocations(databaseOperation) {
	//
	...
	if (databaseOperation.operationType !== 'insert') {
		LOGGER.error(`WARNING: handleApexLocations with operationType != "insert": [${databaseOperation.fullDocument.transaction.operatorLongID}] type="${databaseOperation.operationType}" transactionId="${databaseOperation.fullDocument.transaction.transactionId}"`);
	}
	...
	const newApexT3Document = simplifyApexLocation(databaseOperation.fullDocument);
		...
		try {
			...
			const updates = flushedData.map((writeOp) => {
				...
				return {
					...
					trip_id: writeOp.data.validation_service_info__journey_id,
				};
			});
