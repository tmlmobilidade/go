...
interface SyncDocumentsOptions<T> {
	...
	pcgiIdKey: string
	pcgiQuery: any
	removeExtraDocuments?: boolean
}

/* * */

export async function syncDocuments<T>({ dbWriter, docParser, flushCallback, goCollection, goIdKey, goQuery, pcgiCollection, pcgiIdKey, pcgiQuery, removeExtraDocuments = true }: SyncDocumentsOptions<T>) {
	try {
		//
		...
		if (removeExtraDocuments && extraDocuments.length > 0) {
