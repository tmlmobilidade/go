...
export function StopsDetailEditNameFormContextProvider({ children }: PropsWithChildren) {
	...
	const { action: handleUpdateName, isLoading: isUpdatingName } = useHandleUpdate({
		fetchFn: async () => await fetchApiData<Stop>({ body: { name }, method: 'PUT', url: API_ROUTES.infrastructure.STOPS_UPDATE(stopId) }),
		onSuccess: (response) => {
			stopsListMutate();
	...
	const contextValue: StopsDetailEditNameFormContextValue = useMemo(() => ({
		actions: {
	...
	return (
		<StopsDetailEditNameFormContext.Provider value={contextValue}>
			{children}
