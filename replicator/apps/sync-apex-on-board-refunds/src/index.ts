		...
		for (const [chunkIndex, chunkData] of allTimestampChunks.entries()) {
			...
			const flushCallback = async (flushedData: MongoDBWriterWriteOps<SimplifiedApexOnBoardRefund>[]) => {
				try {
					...
					LOGGER.info(`Flush [apex_on_board_refunds]: Marked as 'waiting': ${updateRidesResult.modifiedCount} Rides | ${updateSamsResult.modifiedCount} Unique SAMS (${invalidationTimer.get()})`);
			...
			await syncDocuments({
				...
				dbWriter: simplifiedApexOnBoardRefundsDbWritter,

				docParser: parseSimplifiedApexOnBoardRefund,
