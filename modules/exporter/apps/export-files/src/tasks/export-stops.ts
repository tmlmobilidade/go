...
import { parseStops, type StopExportCsvData } from '../utils/parse-stops.js';

/* * */

/**
 * Extracts the unique stop IDs from the export properties.
 * @param properties The stop export properties.
 * @returns The unique stop IDs to export.
 */
function getStopIdsFromExportProperties(properties: StopExportProperties['properties']): StopId[] {
...
export async function exportStopsFile(fileExport: FileExport): Promise<string> {
	...
	//
	// Build the stop IDs from the export properties
	// and resolve the municipality name of each stop
