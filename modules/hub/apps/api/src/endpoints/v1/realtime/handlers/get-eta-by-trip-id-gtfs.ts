 ...
 */
export async function getEtaByTripIdGtfsHandler(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<unknown>) {
	//
	...
	if (!cachedData) {
		Logger.error({ message: `[hub/v1/realtime:getEtaByTripIdGtfsHandler(${request.params.id})] No data in cache.` });
		return reply
