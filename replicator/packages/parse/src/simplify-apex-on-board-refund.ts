...
import { PCGI_ApexTransaction_Refund, type PCGI_TransactionEntity, type SimplifiedApexOnBoardRefund } from '@tmlmobilidade/types';
import { Dates } from '@tmlmobilidade/utils';

/* * */

export function simplifyApexOnBoardRefund(doc: PCGI_TransactionEntity): SimplifiedApexOnBoardRefund {
	//

	//
	// Extract the decoded transaction from the PCGI document.

	const decodedTransaction: PCGI_ApexTransaction_Refund = JSON.parse(doc.decodeValue);
	...
	throw new Error(`Unsupported APEX Refund version: ${decodedTransaction.transactionInfo.apexTransactionVersion}. Expected version 2.0.`);
