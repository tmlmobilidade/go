...
export async function processRawApexTransactionValidation(databaseOperation) {
	...
	try {
		...
		Logger.error({ attributes: { document: databaseOperation.fullDocument }, error, message: `Error transforming APEX Validation: ${databaseOperation.fullDocument.transaction.transactionId}: Reason: ${errorMessage}` });
	}
