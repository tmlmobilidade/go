 ...
 * @param rideAnalysis The ride analysis to test.
 ...
 */
export function testRide(rideAnalysis: Ride['analysis']): [boolean, RideAnalysisSummary] {
	const summaryEntries: [string, RideAnalysis][] = [];

	for (const test of REQUIRED_TESTS) {
		const analysis = rideAnalysis[test] ?? { grade: 'fail', message: 'Test was not found for this ride', reason: test };
	...
	const allRequiredTestsArePass = REQUIRED_TESTS.every((test) => {
		const analysis = rideAnalysis[test];
