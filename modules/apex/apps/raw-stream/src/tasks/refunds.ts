...
export async function processRawApexTransactionRefund(databaseOperation) {
	...
	try {
		...
		Logger.error({ attributes: { document: databaseOperation.fullDocument }, error, message: `Error transforming APEX Refund: ${databaseOperation.fullDocument.transaction.transactionId}: Reason: ${errorMessage}` });
	}
