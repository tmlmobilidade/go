...
export const VehiclesListContextProvider = ({ children }) => {
	...
	const filterByAgencyJoined = useMemo(() => (filterByAgency.length === 0 ? null : filterByAgency.join(';')), [filterByAgency]);
	...
	const applyFiltersToData = () => {
		...
		if (filterByBikesState) {
			filterResult = filterResult.filter(item => (item.bicycles ?? false).toString() === filterByBikesState);
		}

		if (filterByWheelchairState) {
			filterResult = filterResult.filter(item => (item.wheelchair ?? false).toString() === filterByWheelchairState);
		}
	...
	}, [filterBySearchState, filterByAgency, filterByTransports, filterByBikesState, filterByMakeAndModelState, filterByPropulsionState, filterByWheelchairState, vehiclesContext.data.vehicles]);
	...
	const updateFilterByAgency = (values: string[]) => {
		globalSettingsContext.actions.updateFilterByAgency(values);
	};
	...
	const contextValue: VehiclesListContextState = {
		...
		filters: {
			by_agency: filterByAgencyJoined,
