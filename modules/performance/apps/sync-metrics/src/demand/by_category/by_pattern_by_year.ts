...
export const syncDemandByCategoryByPatternByYear = async () => {
	...
	const yearPromises = allTimestampChunks.map((chunkData, chunkIndex) =>
		limit(async () => {
			...
			Logger.info(`Chunk ${chunkIndex + 1}/${allTimestampChunks.length} - Aggregated ${yearlyAgg.length} category-pattern combinations from monthly metrics (${chunkTimer.get()})`);
			return yearlyAgg;
		}),
	);
