...
export function AlertsPublicListCard({ alert, description, title }: AlertsPublicListCardProps) {
	...
	const endDate = alert.active_period_end_date ? Dates.fromUnixTimestamp(alert.active_period_end_date).setZone('Europe/Lisbon', 'offset_only').toFormat('dd/MM/yyyy HH:mm') : t('shared:home.alerts.public.list.card.dates.no_end');
		...
		<article className={styles.card} data-severity={severity}>
			...
			<div className={styles.dates}>
				<div className={styles.dateRow}>
					<span className={styles.dateLabel}>{t('shared:home.alerts.public.list.card.dates.start')}</span>
					<span className={styles.dateValue}>{startDate}</span>
				</div>
				<div className={styles.dateRow}>
					<span className={styles.dateLabel}>{t('shared:home.alerts.public.list.card.dates.end')}</span>
