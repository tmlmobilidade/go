...
export function parseRawApexTransactionLocation(doc: RawApexTransaction): null | SimplifiedApexLocation {
	try {
		//

		//
		// Skip if the transaction version is not supported

		const supportedTransactionVersions = ['location-3.0'];

		if (!supportedTransactionVersions.includes(doc.version)) throw new Error(`Invalid transaction version: "${doc.version}".`);
