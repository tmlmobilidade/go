...
import { type Document, type Filter, type UpdateOptions } from '@tmlmobilidade/go-clients-mongo';
 ...
 */
export async function updateById<T extends Document>(context: GoDbCollectionContext<T>, _id: string, updateFields: UpdatableDocument<T>, options?: UpdateOptions): Promise<T> {
	//
	...
	const existingDocument = await context.collection.findOne({ _id: { $eq: _id as Filter<T>['_id'] } }, options);
	...
	const updateResult = await context.collection.updateOne({ _id: { $eq: _id as Filter<T>['_id'] } }, validatedDocument, options);
	...
	const updatedDoc = await context.collection.findOne({ _id: { $eq: _id as Filter<T>['_id'] } }, options);
