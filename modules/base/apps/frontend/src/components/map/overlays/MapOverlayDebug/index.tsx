...
export function MapOverlayDebug({ className }: Props) {
	//
	// A. Setup variables
	const { ref: { map } } = useMapViewContext();
	...
	const maplibreMap: MapLibre | undefined = useMemo(() => {
		...
		const possibleMap = map?.current as unknown as MapLibre | undefined | { getMap?: () => MapLibre };
