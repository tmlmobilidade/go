...
-- Source: {database}.curr_waypoints (stop lat/lon)
--         {database}.daily_rides           (trip -> hashed_shape_id assignment)
--         {database}.shape_nodes           (per-shape node geometry)
-- Target: {database}.curr_waypoints_snapped
--
...
-- {database}.mv_live_trip_stop_etas so live ETAs can resolve stop -> node_index in O(1).
-- =============================================================================

INSERT INTO {database}.curr_waypoints_snapped
SELECT
...
FROM {database}.curr_waypoints AS w
INNER JOIN {database}.curr_rides     AS d ON w.hashed_trip_id = d.hashed_trip_id
