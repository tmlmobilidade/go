...
-- per processed ride into eta._detect_hist_rides_values, a Join-engine
-- table keyed on _id:
...
UPDATE
    start_time_observed = joinGet('eta._detect_hist_rides_values', 'start_time_observed', _id),
    end_time_observed   = joinGet('eta._detect_hist_rides_values', 'end_time_observed', _id)
WHERE _id IN (SELECT _id FROM eta._detect_hist_rides_values);
