...
(async function init() {
	...
	try {
		...
		Logger.startNodeLogs({ app: 'pt-tmp-unir-ut2-labdb-stream', message: 'Sentry Tracker UT2 LabDb Stream initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
		Logger.error({ error, message: 'Error initializing Sentry Tracker UT2 LabDb Stream' });
	}
	...
	collection
		...
		.on('change', async (change) => {
			...
			if (change.operationType !== 'insert' || !change.fullDocument) {
				Logger.error({ message: `[pt-tmp-unir-ut2-labdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
				return;
			}
