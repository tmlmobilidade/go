...
const writer = new BatchWriter<SimplifiedVehicleEvent>({
	...
	title: `pt-tmp-unir-ut4-labdb-stream`,
});
...
(async function init() {
	...
	try {
		...
		Logger.startNodeLogs({ app: 'pt-tmp-unir-ut4-labdb-stream', message: 'Sentry Tracker CRTM AISA LabDb Stream initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
