...
(async function init() {
	...
	try {
		...
		Logger.startNodeLogs({ app: 'pt-tml-tcb-labdb-stream', message: 'Sentry Tracker TCB LabDb Stream initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
		Logger.error({ error, message: 'Error initializing Sentry Tracker TCB LabDb Stream' });
	}
	...
	const collection = await rawDb.vehicleEvents.ptTmlTcb.getCollection();
	...
	collection
		...
		.on('change', async (change) => {
			...
			if (change.operationType !== 'insert' || !change.fullDocument) {
				Logger.error({ message: `[pt-tml-tcb-labdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
				return;
			}
