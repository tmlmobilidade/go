...
-- (whose ReplacingMergeTree merge cadence makes per-event snapshots unreliable).
--
-- The query mirrors bootstrap/mv-predict-trip-stop-etas.sql (replacing `now64(3)`
-- with the parameter and scoping every CTE to the target trip) and finishes with
...
WITH
    ...
    recent_events AS (
        ...
        WHERE e.trip_id = {trip_id:String}
          AND e.created_at BETWEEN pf.position_created_at - 10 * 60 * 1000
                    ...
                    least(
                        ...
                        + (
                            ...
                            * exp(
                                -greatest(
                                    toFloat64(0),
                                    ({now_ms:Int64} - position_created_at) / 1000.0
                                ) / 180.0
                            )
