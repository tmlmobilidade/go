...
(async function init() {
	...
	try {
		...
		Logger.startNodeLogs({ app: 'es-crtm-la-veloz-labdb-stream', message: 'Sentry Tracker CRTM LA VELOZ LabDb Stream initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
		Logger.error({ error, message: 'Error initializing Sentry Tracker CRTM LA VELOZ LabDb Stream' });
	}
