...
export async function getStopHandler(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<HubV1ApiStop>) {
	...
	if (!cachedData) {
		Logger.error({ message: `[hub/v1/network:getStop(${request.params.id})] No cached data found for stop ${request.params.id}` });
		return sendErrorApiResponse(reply, {
			error: `[hub/v1/network:getStop(${request.params.id})] No cached data found for stop ${request.params.id}`,
			status_code: '404',
		});
	};
