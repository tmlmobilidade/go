...
const writer = new BatchWriter<SimplifiedVehicleEvent>({
	...
	title: `pt-tml-cm-rl-labdb-stream`,
});
...
(async function init() {
	...
	try {
		...
		Logger.startNodeLogs({ app: 'pt-tml-cm-rl-labdb-stream', message: 'Sentry Tracker CRTM AISA LabDb Stream initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
