...
export async function processApexT11(databaseOperation) {
	...
	if (databaseOperation.operationType !== 'insert') {
		LOGGER.error(`WARNING: processApexT11 with operationType != "insert": [${databaseOperation.fullDocument.transaction.operatorLongID}] type="${databaseOperation.operationType}" ${databaseOperation.fullDocument.transaction.transactionId}`);
	}
