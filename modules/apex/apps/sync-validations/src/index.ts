...
const CLICKHOUSE_TABLE = 'simplified_apex_validations';
		...
		for (const [chunkIndex, chunkData] of allTimestampChunks.entries()) {
			...
			const flushCallback = async (flushedData?: SimplifiedApexValidation[]) => {
				...
				try {
					...
					// Logger.info(`Flush [${CLICKHOUSE_TABLE}]: Marked as 'waiting': ${updateRidesResult.modifiedCount} Rides (${invalidationTimer.get()})`);
				}
				catch (error) {
			...
			for await (const pcgiDocument of missingDocumentsStream) {
				const parsedValidation = parseSimplifiedApexValidation(pcgiDocument);
				if (!parsedValidation) continue;

				await clickhouseWriter.write(parsedValidation, undefined, flushCallback);
				syncedCount++;
			}
