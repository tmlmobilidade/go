...
import { useAnnotationsListFilterAgencies } from './filters/AnnotationsListFilterAgencies/use-annotations-list-filter-agencies';
import { useAnnotationsListFilterDates } from './filters/AnnotationsListFilterDates/use-annotations-list-filter-dates';
import { useAnnotationsListFilterSearch } from './filters/AnnotationsListFilterSearch/use-annotations-list-filter-search';

/* * */

interface UseAnnotationsListDataReturnType {
	data: AnnotationNormalized[]
	error: null | string
	isLoading: boolean
...
export function useAnnotationsListData(): UseAnnotationsListDataReturnType {
	//
	...
	const filterDates = useAnnotationsListFilterDates();
	const filterSearch = useAnnotationsListFilterSearch();

	//
	// B. Fetch data

	const { data, error, isLoading, isValidating, mutate } = useSWR<ApiResponse<Annotation[]>>(API_ROUTES.dates.ANNOTATIONS_LIST, {
	...
	const normalizedData = useMemo(() => {
		...
		return data.data.map((item): AnnotationNormalized => ({
			...item,
