...
(async function init() {
	...
	try {
		...
		Logger.startNodeLogs({ app: 'pt-tml-cm-va-labdb-stream', message: 'Sentry Tracker CM VA LabDb Stream initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
		Logger.error({ error, message: 'Error initializing Sentry Tracker CM VA LabDb Stream' });
	}
	...
	const collection = await rawDb.vehicleEvents.ptTmlCmVa.getCollection();
	...
	collection
		...
		.on('change', async (change) => {
			...
			if (change.operationType !== 'insert' || !change.fullDocument) {
				Logger.error({ message: `[pt-tml-cm-va-labdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
				return;
			}
