...
import { type ExtractionTaskContext, type ExtractionTaskResult, type OperationRidesV3Extraction, OperationRidesV3ExtractionPropertiesSchema } from '@tmlmobilidade/go-types-extractions';
...
import { type OperationRidesV3QueryRow } from './types.js';
 ...
 */
export async function operationRidesV3Extraction(context: ExtractionTaskContext, extraction: OperationRidesV3Extraction): Promise<ExtractionTaskResult> {
	//

	//
	// Validate the received properties

	const validatedProperties = OperationRidesV3ExtractionPropertiesSchema.parse(extraction.properties);
	...
	const writer = new BatchWriter<OperationRidesV3QueryRow>({
		batch_size: 10_000,
		insertFn: async (rows) => {
	...
	const stream = queryResult.stream<OperationRidesV3QueryRow>();
