...
/**
 * Hands out the next batch of Core Vehicle Events to a migrate worker instance.
 * Marks the batch with a session ID so no other instance picks the same documents,
 * and returns that session ID (or null when there is nothing to process or the
 * coordinator is busy serving another instance).
 * @param request The request object, carrying the calling instance ID
 */
export async function getCoreVehicleEventsHandler(request: FastifyRequest<{ Params: { processorInstanceId: string } }>): Promise<null | string> {
	//
	...
	try {
		...
		//
		// Mark those Core Vehicle Events with the session ID to ensure the next batch
		// does not include them, and return the session ID to the caller instance.
