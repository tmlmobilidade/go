...
export const syncDemandByCategoryByPatternByYear = async () => {
	//
	...
	const categoryMap = new Map<string, DemandByCategoryByPatternByYear>();
	...
	const yearPromises = allTimestampChunks.map((chunkData, chunkIndex) =>
		limit(async () => {
			...
			Logger.info({ message: `Chunk ${chunkIndex + 1}/${allTimestampChunks.length} - Aggregated ${yearlyAgg.length} category-pattern combinations from monthly metrics (${chunkTimer.get()})` });
			return yearlyAgg;
		}),
	);
