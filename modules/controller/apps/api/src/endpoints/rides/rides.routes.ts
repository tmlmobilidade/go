...
server.register(
	async (instance) => {
		...
		instance.get(
			'/',
			(request: FastifyRequest<{ Querystring: GetRidesBatchQuery }>, reply: FastifyReply<RideNormalized[]>) => RidesSharedController.getBatch(request, reply),
		);
		...
		instance.get(
			'/:id/ride',
			(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<RideNormalized>) => RidesSharedController.getRideById(request, reply),
		);
