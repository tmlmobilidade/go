...
import { type RawVehicleEventPtTmlCmVa, type SimplifiedVehicleEvent } from '@tmlmobilidade/go-types-vehicle-events';
...
const writer = new BatchWriter<SimplifiedVehicleEvent>({
	...
	title: 'pt-tml-cm-va-rawdb-stream',
});

/**
 * Transforms a newly inserted CM VA RawVehicleEvent into a SimplifiedVehicleEvent
 ...
 */
export async function processRawVehicleEvent(change: ChangeStreamDocument<RawVehicleEventPtTmlCmVa>) {
	//
	...
	if (change.operationType !== 'insert' || !change.fullDocument) {
		Logger.error({ message: `[pt-tml-cm-va-rawdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
		return;
	}
