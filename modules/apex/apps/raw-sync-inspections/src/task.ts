...
export async function syncApexInspections(timeChunk: PerformInTimeChunksItem) {
	...
	await replicate<RawApexTransaction>({
		...
		countDestinationDbFn: async () => {
			return await labDb.simplifiedApex.inspections.count(
				'*',
		...
		deleteDestinationDbFn: async (ids: string[]) => {
			await performInChunks(ids, async (chunk) => {
				await labDb.simplifiedApex.inspections.delete(
					'_id IN $1',
					{ 1: chunk },
				);
			}, 250);
		...
		distinctDestinationDbFn: async () => {
			const result = await labDb.simplifiedApex.inspections.distinct(
				'_id',
				'created_at >= $1 AND created_at < $2',
				{ 1: timeChunk.start, 2: timeChunk.end },
			);
		...
		writeSourceDocumentToDestinationDbFn: async (sourceDbDocument) => {
			try {
				...
				Logger.error({ attributes: { document: sourceDbDocument }, error, message: `Error transforming APEX Inspection: ${sourceDbDocument._id} Reason: ${errorMessage}` });
			}
		},
