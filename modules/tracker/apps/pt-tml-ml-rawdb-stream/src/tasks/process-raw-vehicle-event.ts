...
import { type RawVehicleEventPtTmlMl, type SimplifiedVehicleEvent } from '@tmlmobilidade/go-types-vehicle-events';
...
const writer = new BatchWriter<SimplifiedVehicleEvent>({
	...
	title: 'pt-tml-ml-rawdb-stream',
});

/**
 * Transforms a newly inserted Metro Lisboa RawVehicleEvent into a SimplifiedVehicleEvent
 ...
 */
export async function processRawVehicleEvent(change: ChangeStreamDocument<RawVehicleEventPtTmlMl>) {
	//
	...
	if (change.operationType !== 'insert' || !change.fullDocument) {
		Logger.error({ message: `[pt-tml-ml-rawdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
		return;
	}
