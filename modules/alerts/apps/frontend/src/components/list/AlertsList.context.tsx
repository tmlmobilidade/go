...
export function AlertsListContextProvider({ children }: PropsWithChildren) {
	...
	const filterMunicipality = useFilterStateList('municipality', Array.from(locationsContext.data.municipalities.keys()), Array.from(locationsContext.data.municipalities.values()).map(item => ({ label: item.name, value: item.id })));
