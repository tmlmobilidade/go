...
export function DatesSelector() {
	...
	const dates = useMemo(() => holidaysDetailContext.data.form.values.dates ?? [], [holidaysDetailContext.data.form.values.dates]);
	...
	const handleToggleDisplayedMonth = () => {
		if (isAllDisplayedMonthSelected) {
			...
			holidaysDetailContext.data.form.setFieldValue('dates', newDates);
		}
	};
