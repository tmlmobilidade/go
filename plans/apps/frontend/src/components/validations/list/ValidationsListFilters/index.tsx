		...
		<Menu closeOnClickOutside={false} trigger="click-hover" withArrow>
			...
			<Menu.Dropdown classNames={{ dropdown: styles.dropdown }}>
				...
				<div className={styles.filterItem}>
					<DatePicker description="Data de início da vigência do validação" flex={1} label="Data de início" onChange={actions.changeValidFrom} value={filters.validFrom ? Dates.fromOperationalDate(filters.validFrom, 'Europe/Lisbon').js_date : null} clearable />
					<DatePicker description="Data de fim da vigência do validação" flex={1} label="Data de fim" onChange={actions.changeValidUntil} value={filters.validUntil ? Dates.fromOperationalDate(filters.validUntil, 'Europe/Lisbon').js_date : null} clearable />
				</div>
			</Menu.Dropdown>
		</Menu>
	);
