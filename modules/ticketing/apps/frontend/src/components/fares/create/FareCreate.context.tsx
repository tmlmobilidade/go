...
export const FareCreateContextProvider = ({ children }: PropsWithChildren) => {
	...
	const { mutate: allFaresMutate } = useSWR<Fare[]>(API_ROUTES.ticketing.FARES_LIST);
	...
	const { action: handleCreate, isLoading: isSaving } = useHandleUpdate({
		fetchFn: async () => await fetchData<Fare>(API_ROUTES.ticketing.FARES_LIST, 'POST', form.getValues()),
		onSuccess: (createdItem) => {
			form.resetDirty();
			allFaresMutate();
			closeCreateFareModal();
			router.push(keepUrlParams(PAGE_ROUTES.ticketing.FARES_DETAIL(createdItem._id)));
		},
	});
