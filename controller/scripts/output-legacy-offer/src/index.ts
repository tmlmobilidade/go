...
async function createOfferJourneysFromGtfs(filePath: string, startDate: OperationalDate, endDate: OperationalDate): Promise<void> {
	try {
		//
...
(async function init() {
	...
	try {
		...
		LOGGER.error(`--start-date and/or --end-date are not valid: --start-date="${options.startDate}" --end-date="${options.startDate}". They should be in YYYYMMDD format.`, error);
		return;
	}
	...
	await createOfferJourneysFromGtfs(options.file, options.startDate, options.endDate);
})();
