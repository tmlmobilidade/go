...
export class GODBClickHouseService {
	//

	private static _instance: null | Promise<GODBClickHouseService> = null;
	...
	public static async getInstance() {
		...
		if (!this._instance) {
			this._instance = (async () => {
				const instance = new GODBClickHouseService();
	...
	private async connect() {
		Logger.info('Connecting to GODBClickHouseService...');
	...
	private async getConnectionString(): Promise<string> {
		...
		Logger.info('[GODBClickHouseService] Setting up SSH Tunnel...');

		const connection = await this.tunnel.connect();
		const addr = connection.address();

		if (!addr || typeof addr !== 'object') {
			throw new Error('[GODBClickHouseService] Failed to retrieve SSH tunnel address.');
		}
