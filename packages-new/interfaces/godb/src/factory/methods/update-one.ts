...
/**
 * Updates a document by a filter.
 * @param context The context of the Mongo interface.
 * @param filter The filter to use to find the document to update.
 ...
 */
export async function updateOne<T extends Document>(context: GoDbCollectionContext<T>, filter: Filter<T>, updateFields: UpdatableDocument<T>, options?: MinimalOptions): Promise<T> {
	//
	...
	const existingDocument = await context.collection.findOne<T>(filter, { session: options?.session });
	...
	const updateResult = await context.collection.updateOne(filter, { $set: validatedDocument }, { session: options?.session });
	...
	const updatedDoc = await context.collection.findOne<T>(filter, { session: options?.session });
