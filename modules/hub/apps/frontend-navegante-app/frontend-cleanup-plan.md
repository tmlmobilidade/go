...
| 15 | Normalize context flags and memoize provider values | in progress | 10–14 | Alerts, lines, stops, vehicles, trip updates, map, user location, vehicle detail, and all three list contexts now expose stable provider values; local boolean flags use snake_case and permanently undefined/false flags were removed. Route planner and the remaining line/stop detail and analytics providers remain. |
