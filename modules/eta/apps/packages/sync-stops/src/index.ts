/* * */
import { Filter, hashedTrips, rides, stops } from '@tmlmobilidade/interfaces';
...
export async function syncShapeNodes({ batchSize = 100_000, chunkLength = 25, client, ridesQuery }: SyncShapeNodesOptions): Promise<{ shapeNodesProcessed: number }> {
	...
	const distinctHashedShapeIds = await rides.distinct('hashed_trip_id', ridesQuery);
	const hashedShapesCollection = await hashedTrips.getCollection();
