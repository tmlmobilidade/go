...
(async function init() {
	...
	try {
		...
		Logger.startNodeLogs({ app: 'pt-tml-cm-alsa-labdb-stream', message: 'Sentry Tracker CM ALSA LabDb Stream initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
		Logger.error({ error, message: 'Error initializing Sentry Tracker CM ALSA LabDb Stream' });
	}
	...
	collection
		...
		.on('change', async (change) => {
			...
			if (change.operationType !== 'insert' || !change.fullDocument) {
				Logger.error({ message: `[pt-tml-cm-alsa-labdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
				return;
			}
