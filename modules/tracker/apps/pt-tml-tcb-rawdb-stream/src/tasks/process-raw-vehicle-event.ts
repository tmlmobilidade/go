...
import { type RawVehicleEventPtTmlTcb, type SimplifiedVehicleEvent } from '@tmlmobilidade/go-types-vehicle-events';
...
const writer = new BatchWriter<SimplifiedVehicleEvent>({
	...
	title: 'pt-tml-tcb-rawdb-stream',
});

/**
 * Transforms a newly inserted TCB RawVehicleEvent into a SimplifiedVehicleEvent
 ...
 */
export async function processRawVehicleEvent(change: ChangeStreamDocument<RawVehicleEventPtTmlTcb>) {
	//
	...
	if (change.operationType !== 'insert' || !change.fullDocument) {
		Logger.error({ message: `[pt-tml-tcb-rawdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
		return;
	}
