...
const MODULE_CONFIGS: Record<string, Record<Environment, ModuleConfigGroup>> = {
	...
	performance: {
		dev: {
			...
			cors_origin: true,
			frontend_port: 51006,
			frontend_url: 'http://localhost:51006/performance-new',
		},
		prd: {
			api_url: 'https://go.tmlmobilidade.pt/performance/api',
			frontend_url: 'https://go.tmlmobilidade.pt/performance-new',
			...DEFAULT_PRD_CONFIG,
		},
		stg: {
			api_url: `https://${process.env.ENVIRONMENT || process.env.NEXT_PUBLIC_ENVIRONMENT}.go-stg.tmlmobilidade.pt/performance/api`,
			frontend_url: `https://${process.env.ENVIRONMENT || process.env.NEXT_PUBLIC_ENVIRONMENT}.go-stg.tmlmobilidade.pt/performance-new`,
			...DEFAULT_STG_CONFIG,
		},
	},
