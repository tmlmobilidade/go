...
import { type RawVehicleEventPtTmlCmTst, type SimplifiedVehicleEvent } from '@tmlmobilidade/go-types-vehicle-events';
...
const writer = new BatchWriter<SimplifiedVehicleEvent>({
	...
	title: 'pt-tml-cm-tst-rawdb-stream',
});

/**
 * Transforms a newly inserted CM TST RawVehicleEvent into a SimplifiedVehicleEvent
 ...
 */
export async function processRawVehicleEvent(change: ChangeStreamDocument<RawVehicleEventPtTmlCmTst>) {
	//
	...
	if (change.operationType !== 'insert' || !change.fullDocument) {
		Logger.error({ message: `[pt-tml-cm-tst-rawdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
		return;
	}
