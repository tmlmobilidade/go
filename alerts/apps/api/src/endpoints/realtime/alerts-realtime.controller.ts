...
import { alertsRealtime, files } from '@tmlmobilidade/interfaces';
...
export class AlertsRealtimeController {
	/**
	...
	static async create(request: FastifyRequest<{ Body: Alert }>, reply: FastifyReply<Alert>) {
		const result = await alertsRealtime.insertOne(request.body);
	...
	static async delete(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<void>) {
		...
		await alertsRealtime.deleteById(id);
	...
	static async deleteImage(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<void>) {
		...
		const alert = await alertsRealtime.findById(id);
		...
		await alertsRealtime.updateById(id, { file_id: undefined });
	...
	static async getAll(request: FastifyRequest, reply: FastifyReply<Alert[]>) {
		const result = await alertsRealtime.findMany({}, { sort: { created_at: -1 } });
	...
	static async getGtfs(request: FastifyRequest, reply: FastifyReply<ServiceAlertResponse>) {
		const result = await alertsRealtime.findMany({
			$and: [
				{
					$or: [
	...
	static async update(
		...
		const alert = await alertsRealtime.updateById(id, alertData);
	...
	static async uploadImage(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<File>) {
		...
		await alertsRealtime.updateById(id, { file_id: result._id.toString() });
