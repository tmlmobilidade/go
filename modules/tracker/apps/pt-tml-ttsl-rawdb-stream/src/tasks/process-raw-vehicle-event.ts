...
import { type RawVehicleEventPtTmlTtsl, type SimplifiedVehicleEvent } from '@tmlmobilidade/go-types-vehicle-events';
...
const writer = new BatchWriter<SimplifiedVehicleEvent>({
	...
	title: 'pt-tml-ttsl-rawdb-stream',
});

/**
 * Transforms a newly inserted TTSL RawVehicleEvent into a SimplifiedVehicleEvent
 ...
 */
export async function processRawVehicleEvent(change: ChangeStreamDocument<RawVehicleEventPtTmlTtsl>) {
	//
	...
	if (change.operationType !== 'insert' || !change.fullDocument) {
		Logger.error({ message: `[pt-tml-ttsl-rawdb-stream] WARNING: unexpected changeStream document: operationType="${change.operationType}"` });
		return;
	}
