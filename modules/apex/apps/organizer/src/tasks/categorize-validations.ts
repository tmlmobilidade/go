...
import { getSimplifiedApexValidationCategory, validateIfSimplifiedApexValidationIsPassenger } from '@tmlmobilidade/go-apex-pckg-parse';
...
export async function categorizeValidations() {
	try {
		...
		const allValidations = await queryRows<ValidationRow>(`
			SELECT _id, event_type, on_board_refund_id, on_board_sale_id, units_qty, validation_status
			FROM simplified_apex_validations
			ORDER BY created_at DESC
		`);

		for (const validationDocument of allValidations) {
