...
export class AlertsController {
	...
	static async getImage(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<File>) {
		...
		const foundImageFile = await goDb.core.files.findOne({ _id: foundAlert.file_id });
		if (!foundImageFile) {
	...
	static async uploadImage(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply<File>) {
		...
		const fileUploadResult = await goDb.core.files.upload(buffer, {
			created_by: request.me._id,
		...
		if (foundAlert.file_id) {
			await goDb.core.files.deleteById(foundAlert.file_id);
		}
