...
export class PCGITicketingClient {
	...
	private async connect() {
		...
		this.client = new MongoClient(connectionString, {
			...
			replicaSet: process.env.PCGI_TICKETING_RS_NAME,
			retryReads: true,
			retryWrites: true,
	...
	private async getConnectionString(): Promise<string> {
		...
		this.tunnel = getSshTunnel({
			forwardOptions: { dstAddr: process.env.PCGI_TICKETING_HOST_1, dstPort: Number(process.env.PCGI_TICKETING_PORT_1) },
			prefix: 'PCGI',
		});
