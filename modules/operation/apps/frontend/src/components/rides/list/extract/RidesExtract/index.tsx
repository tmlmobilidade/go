...
import { type Extraction, type InfrastructureRidesV1ExtractionCreate } from '@tmlmobilidade/go-types-extractions';
...
export function RidesExtract() {
	...
	const { action: handleExtract } = useHandleAction({
		fetchFn: async () => await fetchApiData<Extraction[], InfrastructureRidesV1ExtractionCreate>({
			body: {
				properties: {
					municipality_ids: [],
				},
