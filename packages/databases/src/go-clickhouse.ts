...
export class GOClickHouseDatabase {
	//

	private static _instance: null | Promise<GOClickHouseDatabase> = null;
	...
	public static async getClient() {
		...
		if (!this._instance) {
			this._instance = (async () => {
				const instance = new GOClickHouseDatabase();
	...
	private async connect() {
		Logger.info('Connecting to GOClickHouseDatabase...');
	...
	private async getConnectionString(): Promise<string> {
		...
		Logger.info('[GOClickHouseDatabase] Setting up SSH Tunnel...');

		const connection = await this.tunnel.connect();
		const addr = connection.address();

		if (!addr || typeof addr !== 'object') {
			throw new Error('[GOClickHouseDatabase] Failed to retrieve SSH tunnel address.');
		}
