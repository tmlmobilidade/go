...
const apexT3DbWritter = new MongoDbWriter<SimplifiedApexOnBoardSale>({
	batch_size: 250,
	batch_timeout: 10000,
...
export async function handleApexOnBoardSales(databaseOperation) {
	//
	...
	if (databaseOperation.operationType !== 'insert') {
		LOGGER.error(`WARNING: handleApexOnBoardSales with operationType != "insert": [${databaseOperation.fullDocument.transaction.operatorLongID}] type="${databaseOperation.operationType}" transactionId="${databaseOperation.fullDocument.transaction.transactionId}"`);
	}
	...
	const newApexT3Document = simplifyApexOnBoardSale(databaseOperation.fullDocument);
