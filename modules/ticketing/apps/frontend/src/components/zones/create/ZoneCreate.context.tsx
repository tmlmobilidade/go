...
export const ZoneCreateContextProvider = ({ children }: PropsWithChildren) => {
	...
	const { mutate: allZonesMutate } = useSWR<Zone[]>(API_ROUTES.ticketing.ZONES_LIST);
	...
	const { action: handleCreate, isLoading: isSaving } = useHandleUpdate({
		fetchFn: async () => await fetchData<Zone>(API_ROUTES.ticketing.ZONES_LIST, 'POST', form.getValues()),
		onSuccess: (createdItem) => {
			form.resetDirty();
			allZonesMutate();
			closeCreateZoneModal();
			router.push(keepUrlParams(PAGE_ROUTES.ticketing.ZONES_DETAIL(createdItem._id)));
		},
	});
