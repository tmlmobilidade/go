...
const apexT3DbWritter = new MongoDbWriter<ApexT3>({
	batch_size: 250,
	batch_timeout: 10000,
	collection: await apexT3.getCollection(),
	idle_timeout: 10000,
});

/* * */

export async function processApexT3(databaseOperation) {
	//
	...
	if (databaseOperation.operationType !== 'insert') {
		LOGGER.error(`WARNING: processApexT3 with operationType != "insert": [${databaseOperation.fullDocument.transaction.operatorLongID}] type="${databaseOperation.operationType}" transactionId="${databaseOperation.fullDocument.transaction.transactionId}"`);
	}
	...
	const newApexT3Document = parseApexT3(databaseOperation.fullDocument);
