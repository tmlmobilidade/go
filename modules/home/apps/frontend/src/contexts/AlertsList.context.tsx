...
export const AlertsListContextProvider = ({ children }: PropsWithChildren) => {
	...
	// 				const stop = stopsById.get(Number(r.parent_id));
	// 				for (const lid of (stop as unknown as StopWithLines)?.line_ids ?? []) set.add(String(lid));
	// 			}
	// 		}
	// 	}
	// 	return [...set].sort();
	...
	}, [searchResultsData, filterAgency.value, filterCause.value, filterEffect.value, agencyIdsInAlerts.length, includePastAlerts, defaultPeriodSince, defaultPeriodUntil, periodSince, periodUntil]);
