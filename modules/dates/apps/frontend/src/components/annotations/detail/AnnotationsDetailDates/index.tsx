...
import { Button, MiniCalendar, Section, Text, TimeChip, useStandardFormWatch } from '@tmlmobilidade/ui';
...
export function AnnotationsDetailDates() {
	//
	...
	const dates = useMemo(() => datesValue ?? [], [datesValue]);

	const isDisabled = !capabilities.editEnabled;
	...
	const initialMonth = useMemo(() => {
		// Prefer showing the month of the most recently selected date; fallback to the current month
		if (dates.length > 0) {
	...
	const setDates = (newDates: OperationalDate[]) => {
		form.setValue('dates', newDates, { shouldDirty: true, shouldValidate: true });
	};

	const handleSelect = (date: Date) => {
		const operationalDate = dayjs(date).format('YYYYMMDD') as OperationalDate;
		if (dates.includes(operationalDate)) setDates(dates.filter(d => d !== operationalDate));
		else setDates([...dates, operationalDate]);
	};

	const handleRemove = (operationalDate: OperationalDate) => {
		setDates(dates.filter(d => d !== operationalDate));
	};

	const handleRemoveMonth = (monthDates: OperationalDate[]) => {
		setDates(dates.filter(d => !monthDates.includes(d)));
	};

	const handleToggleDisplayedMonth = () => {
		if (isAllDisplayedMonthSelected) setDates(dates.filter(d => !displayedMonthDates.includes(d)));
		else setDates([...new Set([...dates, ...displayedMonthDates])].sort() as OperationalDate[]);
	};
				...
				{dates.length > 0 && (
					<div className={styles.monthsList}>
						{Array.from(groupedByMonth.entries()).map(([monthKey, monthDates]) => {
							...
							const monthLabel = new Date(Number(yyyy), Number(mm) - 1, 1).toLocaleDateString('pt-PT', { month: 'long', year: 'numeric' });
							return (
								<Section key={monthKey} gap="xs" padding="none">
									<div className={styles.monthHeader}>
										<Text className={styles.monthLabel} fw={600} size="sm">{monthLabel}</Text>
										{!isDisabled && (
											<button className={styles.monthSmallButton} onClick={() => handleRemoveMonth(monthDates)} type="button">
												Remover mês
											</button>
										)}
									</div>
										...
										{monthDates.map((date) => {
											...
											return (
												<TimeChip
													...
													time={`${jsDate.getDate()} (${weekdayShort})`}
												/>
											);
										})}
									</div>
