...
const main = async () => {
	...
	try {
		...
		Logger.logsNode({ app: 'ccfl-fetch', message: 'Sentry Tracker CCFL Fetch initialized', module: 'tracker', severity: 'info' });
	} catch (error) {
		Logger.error('Error initializing Sentry Tracker CCFL Fetch', error);
	}
