...
export async function getCoreVehicleEvents(): Promise<string[]> {
	...
	try {
		...
		const latestCoreVehicleEvents = await coreVehicleEventsCollection
			.find({ status_2: { $ne: 'processing' } }, { limit: 10_000, projection: { _id: 1 }, sort: { millis: -1 } })
			.toArray();
		...
		await coreVehicleEventsCollection.updateMany({ _id: { $in: latestCoreVehicleEventsIds } }, { $set: { status_2: 'processing' } });
