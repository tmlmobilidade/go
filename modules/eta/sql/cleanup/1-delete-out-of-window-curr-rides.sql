-- Delete out-of-window rides from {database}.curr_rides.
-- 
...
SELECT count() AS rows_to_delete FROM {database}.curr_rides
WHERE
	start_time_scheduled < toUInt64(toUnixTimestamp64Milli(now64(3))) - (toUInt64({window_hours_before:UInt32}) * 60 * 60 * 1000);

-- Delete all out-of-window rides from {database}.curr_rides:

ALTER TABLE {database}.curr_rides
DELETE WHERE
	start_time_scheduled < toUInt64(toUnixTimestamp64Milli(now64(3))) - (toUInt64({window_hours_before:UInt32}) * 60 * 60 * 1000);
