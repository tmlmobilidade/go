...
export class GODBMongoService {
	//

	private static _instance: null | Promise<GODBMongoService> = null;
	...
	public static async getClient() {
		...
		if (!this._instance) {
			this._instance = (async () => {
				const instance = new GODBMongoService();
	...
	private async connect() {
		Logger.info('Connecting to GODBMongoService...');
		const connectionString = await this.getConnectionString();
		this.client = new MongoClient(connectionString);
		this.client.on('close', () => {
			console.warn('[GODBMongoService] Database connection closed unexpectedly.');
		});
		this.client.on('reconnect', () => {
			console.log('[GODBMongoService] Database reconnected.');
		});
	...
	private async getConnectionString(): Promise<string> {
		...
		if (process.env.GODB_MONGO_TUNNEL_ENABLED === 'false') {
			return `http://${process.env.GODB_MONGO_USER}:${process.env.GODB_MONGO_PASSWORD}@${process.env.GODB_MONGO_HOST}:${process.env.GODB_MONGO_PORT}`;
		}
		...
		Logger.info('[GODBMongoService] Setting up SSH Tunnel...');

		const connection = await this.tunnel.connect();
		const addr = connection.address();

		if (!addr || typeof addr !== 'object') {
			throw new Error('[GODBMongoService] Failed to retrieve SSH tunnel address.');
		}

		return `http://${process.env.GODB_MONGO_USER}:${process.env.GODB_MONGO_PASSWORD}@localhost:${addr.port}`;
	}
