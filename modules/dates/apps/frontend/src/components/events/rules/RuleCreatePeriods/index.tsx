...
export function RuleCreatePeriods() {
	...
	const currentPeriodIds = createRuleContext.data.form.values.kind === 'event_replacement'
		? createRuleContext.data.form.values.periodIds || []
		: [];
	...
	const applyPreset = (key: null | PeriodPresetKey) => {
		...
		if (!key) {
			...
			createRuleContext.data.form.setFieldValue('periodIds', undefined);
			return;
		}
		...
		<Section gap="md">
			...
			<SegmentedMultiSelect
				...
				onChange={(selectedPeriods) => {
					if (createRuleContext.data.form.values.kind !== 'event_replacement') return;
					createRuleContext.data.form.setFieldValue('periodIds', selectedPeriods.length > 0 ? selectedPeriods : undefined);
				}}
