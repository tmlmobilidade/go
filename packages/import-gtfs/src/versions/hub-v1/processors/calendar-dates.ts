 ...
 */
export async function processGtfsHubV1CalendarDates(context: ImportGtfsContext<GtfsHubV1SQLTables>): Promise<void> {
	try {
		//
		...
		const parseEachRow = async (data: HubV1GtfsCalendarDates) => {
			//

			//
			// Validate the current row against the proper type

			const validatedData = HubV1GtfsCalendarDatesSchema.parse(data);
			...
			if ('time_range' in context.config && context.config.time_range.date_range?.start && context.config.time_range.date_range?.end) {
				if (validatedData.date < context.config.time_range.date_range.start || validatedData.date > context.config.time_range.date_range.end) return;
			}
			...
			if ('time_range' in context.config && context.config.time_range.discrete_dates?.length) {
				if (!context.config.time_range.discrete_dates.includes(validatedData.date)) return;
			}
