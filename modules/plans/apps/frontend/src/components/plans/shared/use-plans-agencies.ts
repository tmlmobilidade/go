...
import { type PlanAgencyItem, PlanAgencyRequest } from '@tmlmobilidade/go-plans-pckg-types';
...
interface UsePlansAgenciesReturnType {
	data: PlanAgencyItem[]
	error: null | string
	ids: string[]
	isLoading: boolean
 ...
 */
export function usePlansAgencies(query: PlanAgencyRequest): UsePlansAgenciesReturnType {
	//

	//
	// A. Fetch data

	const { data, error, isLoading, isValidating } = useSWR<ApiResponse<PlanAgencyItem[]>>([API_ROUTES.plans.PLANS_AGENCIES, query], {
		fetcher: async ([url, query]) => await fetchApiData<PlanAgencyItem[]>({ body: query, method: 'POST', url }),
