...
# iptables port rules are NOT set here (port numbers come from Terraform).
# -----------------------------------------------------------------------
set -euo pipefail
...
# Make permanent via rc.local (kernel cmdline would require grub update)
...
# Apply immediately during build too
echo 'never' > /sys/kernel/mm/transparent_hugepage/enabled
echo 'never' > /sys/kernel/mm/transparent_hugepage/defrag
