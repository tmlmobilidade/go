...
import { type HubV1GtfsShapesInput, HubV1GtfsShapesSchema } from '@tmlmobilidade/go-types-hub';
...
export async function exportShapesFile(context: ExportGtfsContext, planData: Plan, sqlTables: GtfsSQLTables) {
	...
	for await (const shapesItem of sqlTables.shapes.stream('ORDER BY shape_id, shape_pt_sequence ASC')) {
		...
		const parsedShapesRow: HubV1GtfsShapesInput = {
		...
		const validatedShapesRow = HubV1GtfsShapesSchema.parse(parsedShapesRow);
