...
export function parseSimplifiedApexValidation(pcgiDoc: any): null | SimplifiedApexValidation {
	try {
		...
		if (pcgiDoc.transaction.apexTransactionType !== APEX_VALIDATIONS_SETTINGS.allowed_apex_transaction_type) throw new Error(`Invalid apexTransactionType: "${pcgiDoc.transaction.apexTransactionType}"`);
