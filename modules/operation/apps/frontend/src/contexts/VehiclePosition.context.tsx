...
interface VehiclePositionContextState {
	data: {
		vehiclePosition: HubV1ApiVehiclePosition[]
...
export const VehiclePositionContextProvider = ({ children }: PropsWithChildren) => {
	const { data: fetchedVehiclePositionData, error, isLoading } = useSWR<HubV1ApiVehiclePosition[], Error>({ credentials: 'omit', url: API_ROUTES.hub.VEHICLES_POSITIONS }, { refreshInterval: 5_000 });
