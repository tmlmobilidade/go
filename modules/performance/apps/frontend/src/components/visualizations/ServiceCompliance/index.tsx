...
export function ServiceCompliance({ agency }: { agency?: AgencyType }) {
	...
	const { t } = useTranslation('performance', { keyPrefix: 'ServiceCompliance' });
	...
	return (
		<MetricCard goal="increase" icon={<IconBus />} isLoading={isLoading} previousValue={formattedData.accomplishedRides.last_week} title={t('title')} updatedAt={formattedData.lastUpdated} value={formattedData.accomplishedRides.now}>
			<div className={styles.cardsContainer}>
				...
				<div style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', gap: '16px' }}>
					...
					/>
					<p style={{ textAlign: 'center', width: '250px' }}>{t('validPctLabel')}</p>
				</div>
