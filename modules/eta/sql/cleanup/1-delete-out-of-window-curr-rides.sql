...
-- Boundary uses toUnixMilliseconds64Milli(now()) and hour offset as ms (× 60 × 60 × 1000).
--
-- Parameters:
...
WHERE
	start_time_scheduled < toUInt64(toUnixMilliseconds64Milli(now64(3))) - (toUInt64({window_hours_before:UInt32}) * 60 * 60 * 1000);
