 ...
 */
export async function getTripUpdatesGtfsRtProtobufHandler(request: FastifyRequest, reply: FastifyReply<unknown>) {
	//
	...
	if (!cachedData) {
		Logger.error({ message: '[hub/v1/realtime:getTripUpdatesGtfsRtProtobufHandler()] No data in cache.' });
		return reply
