...
export async function lockRideAcceptance(request: FastifyRequest<{ Body: { is_locked: UpdateRideAcceptanceDto['is_locked'] }, Params: { id: string } }>, reply: FastifyReply<RideAcceptance>) {
	//
	const oldJustificationData = await goDB.operation.rideAcceptances.findByRideId(request.params.id);
	...
	const updateResult = await goDB.operation.rideAcceptances.updateByRideId(request.params.id, { is_locked: request.body.is_locked, updated_by: request.me._id });
