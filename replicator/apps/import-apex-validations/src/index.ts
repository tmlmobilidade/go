...
export async function importApexValidations() {
	try {
		//
		...
		const apexValidationsDbWritter = new MongoDbWriter<ApexValidation>({ batch_size: 100000, collection: apexValidationsCollection });
		...
		for (const [chunkIndex, chunkData] of allTimestampChunks.entries()) {
			...
			const flushCallback = async (flushedData: MongoDBWriterWriteOps<ApexValidation>[]) => {
				try {
					...
					LOGGER.info(`Flush: Marked ${result.modifiedCount} Rides as 'pending' due to new APEX Validations data (${invalidationTimer.get()})`);
				}
				catch (error) {
			...
			await syncDocuments<ApexValidation>({
				...
				slaCollection: apexValidationsCollection,
