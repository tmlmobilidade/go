...
export function AreaCmScreen3() {
	...
	const delays42Parsed = useMemo(() => {
		if (!delaysData || !slaData) return { primary_value: 0, secondary_value: 0, secondary_value_string: '-' };
		return {
			primary_value: delaysData._42_delayed_for_more_than_five_minutes_count / slaData._42_scheduled_rides_until_now,
			primary_value_string: `${Intl.NumberFormat('pt-PT', { maximumFractionDigits: 0 }).format(delaysData._42_delayed_for_more_than_five_minutes_count / slaData._42_scheduled_rides_until_now * 100)}%`,
			secondary_value: delaysData._42_delayed_for_more_than_five_minutes_count,
			secondary_value_string: `${Intl.NumberFormat('pt-PT').format(delaysData._42_delayed_for_more_than_five_minutes_count)}`,
		};
	}, [delaysData, slaData]);
	...
	const delays44Parsed = useMemo(() => {
		if (!delaysData || !slaData) return { primary_value: 0, secondary_value: 0, secondary_value_string: '-' };
		return {
			primary_value: delaysData._44_delayed_for_more_than_five_minutes_count / slaData._44_scheduled_rides_until_now,
			primary_value_string: `${Intl.NumberFormat('pt-PT', { maximumFractionDigits: 0 }).format(delaysData._44_delayed_for_more_than_five_minutes_count / slaData._44_scheduled_rides_until_now * 100)}%`,
			secondary_value: delaysData._44_delayed_for_more_than_five_minutes_count,
			secondary_value_string: `${Intl.NumberFormat('pt-PT').format(delaysData._44_delayed_for_more_than_five_minutes_count)}`,
		};
	}, [delaysData, slaData]);
		...
		<Grid
			...
			cells={[
				...
				<CardDefault
					...
					timestamp={delaysTimestamp}
					title={t('default:areas.cards.delays.title', '', { agency: '42' })}
				...
				<CardDefault
					...
					timestamp={delaysTimestamp}
					title={t('default:areas.cards.delays.title', '', { agency: '44' })}
