...
function searchDocuments<T extends SearchableDocument<T>>(
     ...
     */
	const calculateScore = <T>(doc: T, boost: number | undefined, queryWords: string[], scoring: Partial<Record<KeyWithStringOrStringArrayValue<T>, number>>): number => {
		let totalScore = 0;
	...
	const filteredScoreDocs = sortedScoreDocs.filter(scoredDoc => scoredDoc.score > Math.floor(bestScore ?? 0 / options.threshold) || bestScore === 0);
