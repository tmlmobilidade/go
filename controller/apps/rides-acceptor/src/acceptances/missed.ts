...
export async function createMissedRidesAcceptances() {
	//
	...
	const pipeline: AggregationPipeline<Ride> = [
		{
			$match: {
				$and: [
					{ 'fullDocument.start_time_scheduled': { $gt: Dates.now('Europe/Lisbon').minus({ days: 3 }).unix_timestamp, $lt: millisecondsFromRideStartToNow } },
					{ 'updateDescription.updatedFields.seen_last_at': { $eq: null } },
				],
