...
export const ZoneDetailContextProvider = ({ children, zoneId }: PropsWithChildren<{ zoneId: string }>) => {
	...
	const { mutate: zonesListMutate } = useSWR<Zone[]>(API_ROUTES.ticketing.ZONES_LIST);
	const { data: zoneData, error: zoneError, isLoading: zoneLoading, mutate: zoneMutate } = useSWR<Zone>(API_ROUTES.ticketing.ZONES_DETAIL(zoneId), { refreshInterval: 5000 });
	...
	const { action: handleSave, isLoading: isSaving } = useHandleUpdate({
		fetchFn: async () => await fetchData<Zone>(API_ROUTES.ticketing.ZONES_DETAIL(zoneId), 'PUT', form.getValues()),
	...
	const { action: handleDelete, isLoading: isDeleting } = useHandleUpdate({
		fetchFn: async () => await fetchData<Zone>(API_ROUTES.ticketing.ZONES_DETAIL(zoneId), 'DELETE', zoneData),
		onSuccess: () => {
			form.resetDirty();
			zonesListMutate();
			router.push(keepUrlParams(PAGE_ROUTES.ticketing.ZONES_LIST));
		},
	});

	const { action: handleLock, isLoading: isLocking } = useHandleUpdate({
		fetchFn: async () => await fetchData<Zone>(API_ROUTES.ticketing.ZONES_DETAIL_LOCK(zoneId)),
		onSuccess: (updatedItem) => {
			form.resetDirty();
			zoneMutate(updatedItem);
			zonesListMutate();
