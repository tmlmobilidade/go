...
export function parseSimplifiedApexOnBoardSale(pcgiDoc: any): null | SimplifiedApexOnBoardSale {
	try {
		...
		if (pcgiDoc.transaction.apexTransactionType !== APEX_ON_BOARD_SALES_SETTINGS.allowed_apex_transaction_type) throw new Error(`Invalid apexTransactionType: "${pcgiDoc.transaction.apexTransactionType}"`);

		if (pcgiDoc.transaction.cardPhysicalType !== APEX_ON_BOARD_SALES_SETTINGS.allowed_card_physical_type) throw new Error(`Invalid cardPhysicalType: "${pcgiDoc.transaction.cardPhysicalType}"`);
