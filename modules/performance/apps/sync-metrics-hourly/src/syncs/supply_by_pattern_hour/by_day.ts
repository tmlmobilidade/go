...
import { SupplyByPatternHourByDay } from '@tmlmobilidade/types';
...
export const syncSupplyByPatternHourByDay = async () => {
	Logger.title(`Sync Supply Metrics by Pattern Hour by Day`);
	const globalTimer = new Timer();
	const METRIC = 'supply_by_pattern_hour_by_day';
	...
	if (!calendarJson.length) {
		throw new Error('Calendar data unavailable — cannot build supply_by_pattern_hour_by_day metrics');
	}
	...
	const patternHourMap = new Map<string, SupplyByPatternHourByDay>();
	...
	const dayPromises = allTimestampChunks.map((chunkData, chunkIndex) =>
		limit(async () => {
			...
			try {
					...
					.aggregate([
						{
							$match: {
								...
								pattern_id: { $exists: true, $ne: null },
								start_time_scheduled: { $exists: true, $ne: null },
							},
						},
						{
							$project: {
						...
						},
						{
							$addFields: {
								...
								// pattern_hour = pattern_id + "_" + HH:MM:SS (Europe/Lisbon)
								// e.g. pattern_id=3001_0_1, start_time_scheduled=1757568900000 → "3001_0_1_06:35:00"
								pattern_hour: {
									$concat: [
										{ $toString: '$pattern_id' },
										'_',
						...
						},
						{
							$group: {
								...
								// revenue per trip/day/pattern_hour
				...
				Logger.info({ message: `Chunk ${chunkLabel} DONE - Found ${ridesAgg.length} pattern hours (${chunkTimer.get()})` });
	...
	for (const { dayLabel, ridesAgg } of allChunksResults) {
		...
		for (const patternHourStats of ridesAgg) {
			const pattern_hour = String(patternHourStats._id ?? 'no-pattern-hour');
			const agency_id = String(patternHourStats.agency_id ?? 'no-agency');
			...
			const cost = (Number(patternHourStats.vkms_scheduled ?? 0) * Number(price_per_km ?? 0)) / 1000;
			const revenue = Number(patternHourStats.revenue ?? 0);
			const passengers_observed = Number(patternHourStats.passengers_observed ?? 0);
			const derived = computeSupplyDerivedFields(cost, revenue, passengers_observed);

			patternHourDoc.data[dayLabel] = {
				accomplished_rides: patternHourStats.accomplished_rides,
				cost,
				...
				scheduled_rides: patternHourStats.scheduled_rides,
				vkms_observed: patternHourStats.vkms_observed,
				vkms_scheduled: patternHourStats.vkms_scheduled,
			};
		}
	}
	...
	Logger.info({ message: [
		...
		`Built ${results.length} pattern hour metric documents`,
	] });
	for (const doc of results) {
		Logger.info({ message: `  pattern_hour ${doc.properties.pattern_hour}: ${Object.keys(doc.data).length} days in data` });
	}
