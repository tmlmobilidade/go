...
export async function main() {
	...
	Logger.info({ message: `Processing ${ridesBatch.length} rides... (coordinator: ${fetchCoordinatorTimerResult} | interface: ${fetchRideDocumentsTimer.get()})`, spacesAfter: 1 });
	...
	await runWithConcurrency(ridesBatch, 100, async (rideData, index) => {
		try {
			...
			Logger.info({ message: [
				`[${`${ridesBatch.length - index}/${ridesBatch.length}`.padStart(7)}]`,
				` FETCH: ${fetchAnalysisDataTime.padEnd(10)}`,
				` AUGMENT: ${augmentRideTime.padEnd(10)}`,
				` ANALYZE: ${analyzeRideTime.padEnd(10)}`,
				` TOTAL: ${rideAnalysisTimer.get().padEnd(10)}`,
				rideData._id.padEnd(50),
				`SKIP: ${analysesResult.skip.length} `.padEnd(10),
				`PASS: ${analysesResult.pass.length} `.padEnd(10),
				`FAIL: ${analysesResult.fail.length} `.padEnd(10),
				`ERROR: ${analysesResult.error.length} [${analysesResult.error.join('|')}]`.padEnd(12),
			].join('') });
