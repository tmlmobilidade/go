...
| 21 | Normalize type, route utility, and test hierarchies | complete | 18–20 | `src/types` now contains folders only; shared one-off contracts live in `common`, shared route models remain grouped together, and the context-specific contract is colocated with its provider. Route utilities are grouped into `itinerary`, `planning`, and `presentation`, while all tests live under the app-level `tests` tree. |
| 22 | Deepen the route-planning workflow | complete | 10, 13 | Restored a green app baseline; added latest-request-wins ownership through SWR Mutation; colocated the context contract, consumer hook, cohesive route state, actions, request lifecycle, transitions, and provider in `RoutePlanner.context.tsx`; kept reusable domain models and pure helpers behind their existing seams. |
