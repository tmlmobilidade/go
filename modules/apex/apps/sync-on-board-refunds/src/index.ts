...
const CLICKHOUSE_TABLE = 'simplified_apex_on_board_refunds';
...
async function syncApexOnBoardRefunds() {
	try {
		...
		const clickhouseWriter = new ClickHouseWriter<SimplifiedApexOnBoardRefund>({
			batch_size: 100_000,
		...
		for (const [chunkIndex, chunkData] of allTimestampChunks.entries()) {
			...
			for await (const pcgiDocument of missingDocumentsStream) {
				const parsedOnBoardRefund = parseSimplifiedApexOnBoardRefund(pcgiDocument);
				if (!parsedOnBoardRefund) continue;

				await clickhouseWriter.write(parsedOnBoardRefund, undefined, flushCallback);
				syncedCount++;
			}
