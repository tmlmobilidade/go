 ...
 */
export async function findOne<T extends Document>(context: GoDbCollectionContext<T>, filter: Filter<T>, options?: FindOptions): Promise<null | T> {
	return await context.collection.findOne(filter, options);
}
