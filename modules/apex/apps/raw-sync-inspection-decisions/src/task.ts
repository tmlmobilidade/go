...
export async function syncApexInspectionDecisions(timeChunk: PerformInTimeChunksItem) {
	...
	await replicate<RawApexTransaction>({
		...
		countDestinationDbFn: async () => {
			return await labDb.simplifiedApex.inspectionDecisions.count(
				'*',
		...
		deleteDestinationDbFn: async (ids: string[]) => {
			await performInChunks(ids, async (chunk) => {
				await labDb.simplifiedApex.inspectionDecisions.delete(
					'_id IN $1',
					{ 1: chunk },
				);
			}, 250);
		...
		writeSourceDocumentToDestinationDbFn: async (sourceDbDocument) => {
			try {
				...
				Logger.error({ attributes: { document: sourceDbDocument }, error, message: `Error transforming APEX Inspection Decision: ${sourceDbDocument._id} Reason: ${errorMessage}` });
			}
		},
