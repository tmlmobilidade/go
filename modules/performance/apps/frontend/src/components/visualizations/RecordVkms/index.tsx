...
export default function RecordSupply({ filters, timeView }: { filters: Filters, timeView: 'annual' | 'daily' | 'monthly' }) {
	...
	const { t } = useTranslation('performance', { keyPrefix: 'RecordVkms' });
	...
	return (
		<Surface overflow="visible">
			<Section gap="lg">
				...
				<Grid columns="abc" gap="lg">
					<RecordCard
						description={t('cards.reference.description')}
						title={t('cards.reference.title')}
						value={vkmsContracted}
					/>
					<RecordCard
						description={t('cards.scheduled.description', { count: transformedRecordData.scheduledRides })}
						title={t('cards.scheduled.title')}
						totalValue={vkmsContracted}
						value={transformedRecordData.vmksScheduled}
					/>
					<RecordCard
						description={t('cards.executed.description', { count: transformedRecordData.accomplishedRides })}
						title={t('cards.executed.title')}
						totalValue={vkmsContracted}
