...
/**
 * Runs the sales by agency by operational date aggregation query.
 * for a given timespan (full, realtime, weekly).
 * @param timespan The timespan to run the aggregation query for.
 * @default 'realtime'
 */
export async function runSalesByAgencyByOperationalDate(timespan: typeof AVAILABLE_TIMESPANS[number]) {
	//
