# -----------------------------------------------------------------------
# clickhouse-oci.pkr.hcl
#
# Builds a custom OCI image for ClickHouse with Docker and OS tuning
# pre-installed. The resulting image is used by Terraform in place of
# the raw Ubuntu base image, dramatically speeding up instance boot.
#
# USAGE:
#   packer init .
#   packer build -var-file=../terraform/terraform.tfvars.hcl .
# -----------------------------------------------------------------------

packer {
  required_plugins {
    oracle = {
      source  = "github.com/hashicorp/oracle"
      version = "~> 1.0"
    }
  }
}


# -----------------------------------------------------------------------
# SOURCE — OCI builder
# Creates a temporary instance, runs provisioners, then captures an image.
# -----------------------------------------------------------------------

source "oracle-oci" "clickhouse_base" {
  # Authentication
  tenancy_ocid = var.tenancy_ocid
  user_ocid    = var.user_ocid
  fingerprint  = var.fingerprint
  key_file     = var.private_key_path
  region       = var.region

  # Placement
  compartment_ocid    = var.compartment_ocid
  availability_domain = var.availability_domain
  subnet_ocid         = var.packer_subnet_ocid

  # Temporary build instance settings
  shape = "VM.Standard.A1.Flex"
  shape_config {
    ocpus         = 2
    memory_in_gbs = 12
  }

  # Base image
  base_image_ocid = var.base_image_ocid

  # Inject the SSH public key into the OCI instance so Packer can connect
  metadata = {
    "ssh_authorized_keys" = file(var.ssh_public_key_path)
  }

  # Packer SSH communicator — connects using the corresponding private key
  ssh_username         = "ubuntu"
  ssh_private_key_file = var.ssh_private_key_path

  # Output image name — datestamped for traceability
  image_name = "clickhouse-base-{{isotime \"2006-01-02\"}}"

  # Tag it so Terraform can discover it via data source
  tags = {
    "PackerBuilt" = "true"
    "ImageType"   = "clickhouse-base"
    "ManagedBy"   = "packer"
  }
}


# -----------------------------------------------------------------------
# BUILD — Run provisioner scripts inside the temporary instance
# -----------------------------------------------------------------------

build {
  name    = "clickhouse-base"
  sources = ["source.oracle-oci.clickhouse_base"]

  # 1. OS performance tuning + install prerequisite packages
  provisioner "shell" {
    script = "${path.root}/scripts/os-tuning.sh"
    execute_command = "sudo bash '{{.Path}}'"
  }

  # 2. Install Docker Engine
  provisioner "shell" {
    script = "${path.root}/scripts/install-docker.sh"
    execute_command = "sudo bash '{{.Path}}'"
  }

  # 3. Clean up apt cache to reduce image size
  provisioner "shell" {
    inline = [
      "sudo apt-get clean",
      "sudo rm -rf /var/lib/apt/lists/*",
      "sudo cloud-init clean --logs",
    ]
  }

  post-processor "manifest" {
    output     = "${path.root}/packer-manifest.json"
    strip_path = true
  }
}
