...
export const FareDetailContextProvider = ({ children, fareId }: PropsWithChildren<{ fareId: string }>) => {
	...
	const { mutate: faresListMutate } = useSWR<Fare[]>(API_ROUTES.ticketing.FARES_LIST);
	const { data: fareData, error: fareError, isLoading: fareLoading, mutate: fareMutate } = useSWR<Fare>(API_ROUTES.ticketing.FARES_DETAIL(fareId), { refreshInterval: 5000 });
	...
	const { action: handleSave, isLoading: isSaving } = useHandleUpdate({
		fetchFn: async () => await fetchData<Fare>(API_ROUTES.ticketing.FARES_DETAIL(fareId), 'PUT', form.getValues()),
	...
	const { action: handleDelete, isLoading: isDeleting } = useHandleUpdate({
		fetchFn: async () => await fetchData<Fare>(API_ROUTES.ticketing.FARES_DETAIL(fareId), 'DELETE', fareData),
		onSuccess: () => {
			form.resetDirty();
			faresListMutate();
			router.push(keepUrlParams(PAGE_ROUTES.ticketing.FARES_LIST));
		},
	});

	const { action: handleLock, isLoading: isLocking } = useHandleUpdate({
		fetchFn: async () => await fetchData<Fare>(API_ROUTES.ticketing.FARES_DETAIL_LOCK(fareId)),
		onSuccess: (updatedItem) => {
			form.resetDirty();
			fareMutate(updatedItem);
			faresListMutate();
